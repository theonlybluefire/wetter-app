...
export default () => {
  ...
  useEffect(() => {
    if (geocodingQuery.data && geocodingQuery.data.results && location !== prevLocationRef.current) {
      ...
      classesFirstOutputDiv.current = 'h-full w-full fixed bottom-0'
      setResults(
        geocodingQuery.data.results.map((item) => (
          <div className='w-full flex justify-center'>
            <div className='w-1/2 rounded-3xl h-20 bg-rose-600'>
        ...
        <div className='fixed bottom-0 right-0 left-0 md:py-4 py-1 flex md:justify-center'>
          <div className='bg-dark-blue/40 md:w-2/3 w-full h-16 rounded-2xl flex justify-center items-center space-x-2'>
            ...
            <motion.button
              ...
              type='submit'
              className='relative w-1/4 bg-dark-blue z-10 h-12 text-smooth-white font-bold'
            >
              Submit
            </motion.button>
          </div>
